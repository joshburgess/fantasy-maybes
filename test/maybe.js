...
exports.maybe = {
  ...
  'when creating Nothing with value should set value on Nothing': function(
    test
  ) {
  ...
  'when creating Nothing and mapping value should map to correct value': function(
    test
  ) {
    test.ok(
      λ.equals(Maybe.Nothing.map(λ.error('Failed if called')))(Maybe.Nothing)
    )
    test.done()
  },
  ...
  'when creating Just should be Just': λ.check(
    a => {
      return Maybe.Just(a).fold(λ.constant(true), λ.constant(false))
    },
    [λ.AnyVal]
  ),
  'when creating Just and folding should map to correct value': λ.check(
    a => {
      var result = Maybe.Just(a).fold(identity, λ.error('Failed if called'))
      return result === a
    },
    [λ.AnyVal]
  ),
...
exports.maybeT = {
  ...
  'Identity (Applicative)': applicative.identity(λ)(
    Maybe.MaybeT(Identity),
    run
  ),
  'Composition (Applicative)': applicative.composition(λ)(
    Maybe.MaybeT(Identity),
    run
  ),
  'Homomorphism (Applicative)': applicative.homomorphism(λ)(
    Maybe.MaybeT(Identity),
    run
  ),
  'Interchange (Applicative)': applicative.interchange(λ)(
    Maybe.MaybeT(Identity),
    run
  ),
  ...
  'Composition (Functor)': functor.composition(λ)(
    Maybe.MaybeT(Identity).of,
    run
  ),
